...
void CMFCcimageDlg::OnBnClickedBtnImage()
{
	...
	memset(fm, 0xff, nWidth * nHeight);			//메모리를 어떠한 값으로 생성하겠다.
...
CString g_strFileImage = (_T("c:\\image\\save.bmp"));		// 글로벌 함수
...
void CMFCcimageDlg::moveRect()
{
	...
	//memset(fm, 0xff, nWidth * nHeight);	// 이미지를 항상 클리어하고 다시 그리도록 해줌
