...
{
	//프로젝트 - 클래스 마법사 - 메시지 - WM_ERASEBKGND - 처리기 추가 - 적용
...
void CMFCBitmapBtnDlg::InitButtons()
{
	...
	//GetDlgItem(IDC_BTN_ON_OFF)->GetClientRect(&rect);	// 설정한 위치에 맞게 출력
	m_pBtnOnOff = new CBitmapButton;
	m_pBtnOnOff->Create(NULL, WS_CHILD | WS_VISIBLE | BS_OWNERDRAW, rect, this, IDC_BTN_ON_OFF);	//클래스 메모리 포인터로 생성했을 경우
	m_pBtnOnOff->LoadBitmaps(IDB_ON, IDB_OFF);	// (디폴트 버튼, 포커스 버튼, 셀렉트 버튼)
	m_pBtnOnOff->SizeToContent();	// 버튼 사이즈에 맞게 맞춤
}

void CMFCBitmapBtnDlg::OnDestroy()
{
	// 릭 제거 하는 함수 -> 릭(메모리를 할당해놓고 제대로 제거하지 않은 상태에서 종료한 후 남은 것)은 항상 바로바로 정리해야함
	// 릭이 발생하는 이유 InitButtons의 new가 있으면 delete를 해주어야 하는데 이 delete를 이 함수에서 해줌
	//프로젝트 - 클래스 마법사 - 메시지 - WM_DESTROY - 처리기 추가 - 적용
	CDialogEx::OnDestroy();
